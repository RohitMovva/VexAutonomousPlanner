...
def cubic_bezier_point(p0, p1, p2, p3, t):
    return (
        (1 - t) ** 3 * point_to_array(p0)
        + 3 * (1 - t) ** 2 * t * point_to_array(p1)
        + 3 * (1 - t) * t**2 * point_to_array(p2)
        + t**3 * point_to_array(p3)
    )


def cubic_bezier_derivative(p0, p1, p2, p3, t):
    return 3 * (
        (1 - t) ** 2 * (point_to_array(p1) - point_to_array(p0))
        + 2 * (1 - t) * t * (point_to_array(p2) - point_to_array(p1))
        + t**2 * (point_to_array(p3) - point_to_array(p2))
    )


def cubic_bezier_second_derivative(p0, p1, p2, p3, t):
    return 6 * (
        (1 - t) * (point_to_array(p2) - 2 * point_to_array(p1) + point_to_array(p0))
        + t * (point_to_array(p3) - 2 * point_to_array(p2) + point_to_array(p1))
    )
