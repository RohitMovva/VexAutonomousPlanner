...
def verify_curvature_derivative(manager, t_vals, curvatures, curvature_derivatives, title=""):
    """
    Verify that the curvature derivative matches the numerical derivative of curvature.
    
    Args:
        manager: QuinticHermiteSplineManager instance
        t_vals: Parameter values
        curvatures: Curvature values
        curvature_derivatives: Analytical curvature derivatives
        title: Plot title
    """
    ...
    # Calculate numerical derivative of curvature with respect to arc length
    numerical_derivatives = np.gradient(curvatures, arc_lengths)
    
    # Compare with analytical derivatives
    rms_error = np.sqrt(np.mean((numerical_derivatives - curvature_derivatives)**2))
    max_error = np.max(np.abs(numerical_derivatives - curvature_derivatives))
    
    print(f"\nCurvature Derivative Verification ({title}):")
    print(f"RMS Error between analytical and numerical derivatives: {rms_error:.6f}")
    print(f"Maximum Error: {max_error:.6f}")
    
    # Plot comparison
    plt.figure(figsize=(10, 6))
    plt.plot(arc_lengths, curvature_derivatives, 'b-', label='Analytical dκ/ds')
    plt.plot(arc_lengths, numerical_derivatives, 'r--', label='Numerical dκ/ds')
    plt.grid(True)
    plt.legend()
    plt.title(f'Curvature Derivative Comparison ({title})')
...
def verify_curvature_derivative_relationship(manager, t_vals, curvatures, title=""):
    """
    Verify that the curvature derivative matches the numerical derivative of curvature.
    Plots and compares the analytical derivative from get_curvature_derivative()
    against numerical differentiation of get_curvature().
    """
    # Calculate arc lengths for each parameter value
    arc_lengths = np.zeros_like(t_vals)
    for i in range(1, len(t_vals)):
    ...
    # Get analytical curvature derivatives
    analytical_derivatives = np.array([manager.get_curvature_derivative(t) for t in t_vals])
    
    # Calculate numerical derivatives of curvature with respect to arc length
    numerical_derivatives = np.gradient(curvatures, arc_lengths)
    
    # Calculate error metrics
    rms_error = np.sqrt(np.mean((analytical_derivatives - numerical_derivatives)**2))
    max_error = np.max(np.abs(analytical_derivatives - numerical_derivatives))
    ...
    # Plot comparison
    plt.figure(figsize=(10, 6))
    plt.plot(arc_lengths, analytical_derivatives, 'b-', label='Analytical dκ/ds')
    plt.plot(arc_lengths, numerical_derivatives, 'r--', label='Numerical dκ/ds')
    plt.grid(True)
    plt.legend()
    plt.title(f'Curvature Derivative Comparison ({title})')
...
def test_circle_path():
    ...
    # Check curvature derivative
    print(f"Mean curvature derivative: {np.mean(curvature_derivatives):.6f}")
    print(f"Expected curvature derivative: 0.0")
    print(f"Curvature derivative std dev: {np.std(curvature_derivatives):.6f}")
    ...
    # Plot curvature
    ax3.plot(t_vals, curvatures)
    ax3.axhline(y=expected_curvature, color='r', linestyle='--', label='Expected')
    ax3.set_title('Curvature vs Parameter')
    ax3.grid(True)
    ax3.legend()
    
    # Plot curvature derivative
    ax4.plot(t_vals, curvature_derivatives)
    ax4.axhline(y=0, color='r', linestyle='--', label='Expected')
    ax4.set_title('Curvature Derivative vs Parameter')
    ax4.grid(True)
    ax4.legend()

    curvature_derivative = np.array([manager.get_curvature_derivative(t) for t in t_vals])
    ax5.plot(t_vals, curvature_derivative)
    ax5.set_title('Curvature Derivative vs Parameter')
    ax5.grid(True)
    
    # Add numerical vs analytical comparison
    numerical_derivative = np.gradient(curvatures, t_vals)
    ax6.plot(t_vals, curvature_derivative, 'b-', label='Analytical')
    ax6.plot(t_vals, numerical_derivative, 'r--', label='Numerical')
    ax6.set_title('Curvature Derivative Comparison')
    ax6.grid(True)
    ax6.legend()
    ...
    # Verify curvature derivative
    verify_curvature_derivative_relationship(manager, t_vals, curvatures, "Circle Path")
...
def test_s_curve():
    ...
    print(f"Max curvature derivative: {np.max(curvature_derivatives):.6f}")
    print(f"Min curvature derivative: {np.min(curvature_derivatives):.6f}")
    ...
    # Verify curvature derivative
    verify_curvature_derivative_relationship(manager, t_vals, curvatures, "S-Curve")
