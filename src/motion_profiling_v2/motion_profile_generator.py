...
def forward_backward_pass(
    ...
    max_accels = [
        spline_manager.nodes[0].max_acceleration
        if spline_manager.nodes[0].max_acceleration > 0
        else constraints.max_acc
    ]

    if spline_manager.nodes[0].max_velocity > 0:
        max_velocity = spline_manager.nodes[0].max_velocity
    ...
    while current_dist < total_dist:
        ...
        if (
            ...
            if spline_manager.action_points[action_idx].max_velocity > 0:
                max_velocity = spline_manager.action_points[action_idx].max_velocity
            else:
                max_velocity = constraints.max_vel
            ...
            if spline_manager.action_points[action_idx].max_acceleration > 0:
                max_accels.append(
                    spline_manager.action_points[action_idx].max_acceleration
                )
            else:
                max_accels.append(constraints.max_acc)
    ...
    for i in range(len(velocities) - 1):
        ...
        if abs(curvature) < 1e-6:
            ...
            max_accel = min(
                max_accel_ang, max_accel_kin, max_accel_wheel, constraints.max_acc
            )
    ...
    for i in range(len(velocities) - 1, 0, -1):
        ...
        if abs(curvature) < 1e-6:
            ...
            if max_accel_wheel < 0:
                max_accel_wheel = 0
            max_decel = min(
                max_decel_ang, max_decel_kin, max_accel_wheel, constraints.max_dec
            )
...
def generate_motion_profile(
    ...
    while current_pos < total_length:
        ...
        if t % 1 < prev_t % 1 and t < end_param:
            ...
            if current_node.turn != 0:
                current_time = handle_turn(
                    np.radians(current_node.turn), headings[-1], current_time, dt
                )
        ...
        accel = np.clip(
            (target_vel - current_vel) / dt, -constraints.max_dec, constraints.max_acc
        )
        ...
        left_wheel_vels.append(
            current_vel - (angular_vel * constraints.track_width / 2)
        )
        right_wheel_vels.append(
            current_vel + (angular_vel * constraints.track_width / 2)
        )
        current_time += dt
    ...
    logger.info(
        f"Other lists generation took {other_lists_end_time - other_lists_start_time} seconds"
    )
